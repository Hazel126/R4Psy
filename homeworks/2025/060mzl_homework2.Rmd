...
# 也可以用基础包，输入路径读取
Q1 <- read.csv("D:/AACode/R/R4Psy/data/penguin/penguin_rawdata.csv")
```
...
```{r homework 5}
Q5 <- Q4
Q5$health <- factor(Q5$health, levels = c("Poor", "Fair", "Good", "Very Good", "Excellent"))
Q5 <- Q5[order(Q5$health), ]
print(Q5)
```
---
...
```{r}
Q13 <- Q10 %>%
  filter(health %in% c("Good", "Excellent"), phone_total >= 36)
```
